...
class TestValidation < Minitest::Test
  ...
  def test_one_var_with_coefs
    assert_silent { @poly_differ.send(:validation_check, "2x") }
    assert_silent { @poly_differ.send(:validation_check, "-2x") }
    assert_silent { @poly_differ.send(:validation_check, "2.2x") }
    assert_silent { @poly_differ.send(:validation_check, "-2.9x") }
  end

  def test_one_var_with_coefs_oper
    assert_silent { @poly_differ.send(:validation_check, "4x+5x") }
    assert_silent { @poly_differ.send(:validation_check, "4x-5x") }
    assert_silent { @poly_differ.send(:validation_check, "-4.98x-5.57x") }
    assert_silent { @poly_differ.send(:validation_check, "4.74x+57.98x") }
  end

  def test_one_var_with_degree
    assert_silent { @poly_differ.send(:validation_check, "x^0") }
    assert_silent { @poly_differ.send(:validation_check, "x^1") }
    assert_silent { @poly_differ.send(:validation_check, "x^2") }
    assert_silent { @poly_differ.send(:validation_check, "x^3") }
  end

  def test_one_var_with_degree_oper
    assert_silent { @poly_differ.send(:validation_check, "x^2+x^3") }
    assert_silent { @poly_differ.send(:validation_check, "x^2-x^3") }
    assert_silent { @poly_differ.send(:validation_check, "x^2+x^3+x^6") }
    assert_silent { @poly_differ.send(:validation_check, "-x^2-x^3-x^6") }
    assert_silent { @poly_differ.send(:validation_check, "-x^2+x^3-x^6") }
    assert_silent { @poly_differ.send(:validation_check, "x^2-x^3+x^6") }
  end
  ...
  def test_one_var_deg_coefs_oper
    assert_silent { @poly_differ.send(:validation_check, "12x^2+0x^3") }
    assert_silent { @poly_differ.send(:validation_check, "-1x^2-244x^3") }
    assert_silent { @poly_differ.send(:validation_check, "2x^2+48x^3+712x^1") }
    assert_silent { @poly_differ.send(:validation_check, "12x^2-164x^3-9x^6") }
    assert_silent { @poly_differ.send(:validation_check, "-12x^1+164x^3-9x^5") }
    assert_silent { @poly_differ.send(:validation_check, "12x^2-164x^0+9x^6") }
    assert_silent { @poly_differ.send(:validation_check, "12.678x^10+0.001x^3") }
    assert_silent { @poly_differ.send(:validation_check, "-1.2x^2-244.44x^16") }
    assert_silent { @poly_differ.send(:validation_check, "2.2x^2+48.34x^3+712.0x^1") }
    assert_silent { @poly_differ.send(:validation_check, "-12.89x^2-164.18x^3-9.2x^4") }
    assert_silent { @poly_differ.send(:validation_check, "-12.87x^2+164.0x^3-9x^6+x-12") }
    assert_silent { @poly_differ.send(:validation_check, "13.76+12.08x^2-164.0001x^3+913.9x^6-x") }
  end
